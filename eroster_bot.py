...
def chope():
    ...
    time.sleep(0.5)
    try:
        box = browser.find_elements_by_xpath("//input[@type='checkbox']")
    ...
    for each in box:
        if not each.is_selected() and each.is_enabled():
            each.click()
            try:
    ...
    if choped != 0:
        ...
        ctypes.windll.user32.MessageBoxW(0, "Pls check the slot(s) booked", "ALERT", 1)
    else:
        print('nothing')
